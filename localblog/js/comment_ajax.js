	...
	function addGetLooper(area){
		...
		commentgetter.onreadystatechange = function () {
			if (this.readyState==4 && this.status==200) {
					...
					$('#comment'+comment_id).prepend("<a href=''><span id='delcom"+ comment_id +"' class='deletecomment'>Remove</span></a>");
					(function(myid){
						$('#delcom'+myid).click(function(event){
							event.preventDefault();
							$('#comment'+myid).remove();
							my_canary = $('#canary').val();
							$.get('deletecomment.php', {id : myid , canary : my_canary});
						});
					})(comment_id);
            			}
